...
class BioPilotWorkflow:
    ...
    def __init__(self, spark, config_dict: Dict, enable_agent: bool = True,
        ...
        # TODO: implement gameplay dynamics
    ...
    def inject_scenario_faults(self, scenario: str = "overfeed"):
        ...
        if scenario in self.simulation.fault_manager.fault_templates:
            # TODO: make the start time dynamic
    ...
    def run_with_monitoring(self, base_feed_rate: float = 0.1, save_to_lake: bool = True) -> Dict:
        ...
        if self.enable_anomaly_detection:
            ...
            for idx, row in df_obs.iterrows():
                signals = {'X': row['X'],'S_glc': row['S_glc'],'P': row['P'],
                           'DO': row['DO'],'pH': row['pH']}
                time = row['time']
        ...
        # agentic analysis of telemetry data...
        agent_actions = []
        if self.enable_agent:
            print("\nAgent copilot analyzing telemetry...")
            
            # TODO: make rolling window size dynamic
            window_size = 5
            
            for idx, row in df_obs.iterrows():
                time = row['time']
                telemetry = {'X': row['X'],'S_glc': row['S_glc'],'P': row['P'],'DO': row['DO'],
                             'pH': row['pH'],'time': time}
                
                # find and track anomalies for the current scenario
                recent_anomalies = [a for a in self.all_anomaly_scores if abs(a.time - time) < window_size * self.config['SIMULATION_PARAMS']['dt']]
                observation = AgentObservation(time=time,telemetry=telemetry,recent_anomalies=recent_anomalies,
                                               recent_actions=agent_actions[-5:] if agent_actions else [], available_budget=self.agent_budget.copy())
                
                # agentic process: agent analyzes telemetry data and returns a likely root cause and recommended actions
                # classification of actions by criticality
                actions = self.copilot.process_observation(observation)
                
                # agentic process" agent executes recommended actions
                for action in actions:
                    if action.priority >= 4:  # execute critical actions
                        agent_actions.append(action)
                        
                        # update assay budget (gameplay component)
        ...
        if save_to_lake:
            ...
            self.data_lake.save_telemetry(self.spark, self.run_id, true_history, is_observed=False, batch_id=0)
            self.data_lake.save_telemetry(self.spark, self.run_id, observed_history,is_observed=True, batch_id=0)
            if self.enable_anomaly_detection:
            ...
            if self.enable_agent and agent_actions:
                for action in agent_actions:
                    ...
                    self.data_lake.save_agent_action(self.spark,run_id=self.run_id,action_id=action_id,time=action.time,
                                                     action_type=action.action_type.value,parameters=action.parameters,rationale=action.rationale)
            end_timestamp = datetime.now()
            final_titer_raw = df_true['P'].iloc[-1]
            # TODO: implement more customizable scoring and success criteria for product titer
            final_titer = 0.0 if (final_titer_raw is None or pd.isna(final_titer_raw)) else float(final_titer_raw)
            self.data_lake.save_run_metadata(self.spark,run_id=self.run_id,config=self.config,scenario="standard",
                                             final_titer=final_titer, num_anomalies=len([a for a in self.all_anomaly_scores if a.is_anomaly]),
                                             num_actions=len(agent_actions),success=final_titer > 5.0, score=final_titer * 10.0,start_time=start_timestamp,end_time=end_timestamp)
            print(f"Data saved to data lake with run_id: {self.run_id}")
        
        # synthesize simulation summary
        summary = {
            'run_id': self.run_id,
...
def run_example_workflow(spark):
    ...
    Generic example workflow with a generic overfeed situation.
    """
    ...
    # initialize workflow
    workflow = BioPilotWorkflow(spark=spark,config_dict=config,enable_agent=True,enable_anomaly_detection=True)
    
    # inject simple overfeed scenario fault and start simulation
...
def run_batch_scenarios(spark, scenarios: List[str], num_replicates: int = 3) -> pd.DataFrame:
    """
    ...
    for scenario in scenarios:
        ...
        for replicate in range(num_replicates):
            ...
            # ensure randomness across simulations
            config['SIMULATION_PARAMS']['random_seed'] = 42 + replicate
            
            workflow = BioPilotWorkflow(spark=spark,config_dict=config,enable_agent=True,enable_anomaly_detection=True)
            
            # inject scenario faults based on parameters
            if scenario != "standard":
                workflow.inject_scenario_faults(scenario=scenario)
            
            # run the simulation with the fault
            results = workflow.run_with_monitoring(base_feed_rate=0.1,save_to_lake=True)
            
            # collect run summary stats
            all_results.append({
...
def visualize_run(results: Dict, save_path: Optional[str] = None):
    ...
    for i, (sig, title) in enumerate(zip(signals, titles)):
        ...
        # true vs. observed
        # where true is derived from statistical simulation and observed contains stochasticity and injected faults
        ax.plot(df_true['time'], df_true[sig], label='True', linewidth=2, alpha=0.8)
        ax.plot(df_obs['time'], df_obs[sig],label='Observed', linestyle='--', alpha=0.7)
        ...
        if results['anomaly_scores']:
            ...
            if sig_anomalies:
                ...
                anomaly_values = [df_obs[df_obs['time'] == t][sig].values[0] for t in anomaly_times if len(df_obs[df_obs['time'] == t]) > 0]
                ax.scatter(anomaly_times, anomaly_values,color='red', marker='x', s=100,label='Anomaly', zorder=5)
    ...
    # subplot of summary text for run
    axes[-1].axis('off')
    summary_text = f"""
    RUN SUMMARY
    {'='*30}
    ...
    axes[-1].text(0.1, 0.5, summary_text, fontsize=11, family='monospace',verticalalignment='center')
    
    plt.suptitle(f"BioPilot Run: {results['run_id']}", fontsize=16, fontweight='bold')
    plt.tight_layout()
...
# main execution of simulation
if __name__ == "__main__":

    print("BioPilot Workflow - Complete Integration Demo")
    print("="*60)
    
    # basic example - single run with default overfeed fault
    ...
    # batch example - multiple batch runs with varying fault injection scenarios
